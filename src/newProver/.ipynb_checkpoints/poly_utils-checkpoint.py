# https://github.com/ethereum/research/blob/master/polynomial_reconstruction/poly_utils.py
...
class PrimeField():
    ...
    def add_polys(self, a, b, init_order_a = 0, init_order_b = 0):
        ...
        return ([((a[i+init_order_a-init_order_result] if (i+init_order_a-init_order_result < len(a) and i+init_order_a-init_order_result >= 0) else 0) + (b[i+init_order_b-init_order_result] if (i+init_order_b-init_order_result < len(b) and i+init_order_b-init_order_result >= 0) else 0))
                % self.modulus for i in range(max(len(a)+init_order_a, len(b)+init_order_b))], init_order_result)
